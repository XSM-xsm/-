...
~~~
 ...
 ~~~css
数字 400 等价于 normal，而 700 等价于 bold。  但是更喜欢用数字来表示。  
 ~~~
...
~~~css
和类选择器相区别，  类选择器是一个点 比如 .demo {}   而我们的伪类 用 2个点 就是 冒号  比如  :link{}
~~~
...
- :active    /* 选定的链接 */
   ...
   注意写的时候，顺序尽量不要颠倒，按照  lvha 的顺序。 love   hate爱上了讨厌 记忆法    或者   lv 包包 非常 hao 
...
CSS的三个大模块：  盒子模型 、 浮动 、 定位。  

所谓盒子模型就是把HTML页面中的元素看作是一个矩形的盒子，也就是一个盛装内容的容器。每个矩形都由元素的内容、内边距（padding）、边框（border）和外边距（margin）组成。

## 网页布局的本质

行内元素比如 文字 类似牛奶，需要一个盒子把他们装起来，有了盒子，可以随意的，自由的，摆放位置。
...
3. width   没有问题经常使用宽度剩余法 高度剩余法来做。



## CSS3盒模型

CSS3中可以通过box-sizing 来指定盒模型，即可指定为content-box、border-box，这样计算盒子大小的方式就发生了改变。
...
后来发现浮动可以让任何盒子可以一行排列,因此用浮动的特性来布局。
...
浮动本质是用来做一些文字混排效果的，但是被拿来做布局用，则会有很多的问题出现， 但是，不能说浮动不好   
...
在CSS中有三个显示和隐藏的单词比较常见，要区分开，他们分别是 display visibility 和 overflow。

他们的主要目的是让一个元素在页面中消失，但是不在文档源码中删除。 最常见的是网站广告，当点击类似关闭不见了，但是重新刷新页面，它们又会出现
...
有个很重要特性要记住： 图片或者表单等行内块元素，他的底线会和父级盒子的基线对齐。这样会造成一个问题，就是图片底侧会有一个空白缝隙。
...
CSS 精灵其实是将网页中的一些背景图像整合到一张大图中（精灵图），要做的，就是把小图拼合成一张大图。
...
这个是阿里M2UX的一个icon font字体图标字库，包含了淘宝图标库和阿里妈妈图标库。可以使用AI制作图标上传生成。

**fontello**

[http://fontello.com/](http://fontello.com/)

在线定制icon font字体图标字库，也可以直接从GitHub下载整个图标集，该项目也是开源的。
...
上传完毕， 网站会把UI做的svg图片转换为字体格式， 然后下载下来就好了
...
1. 首先把 以下4个文件放入到 fonts文件夹里面。 通俗的做法
   ...
   如果工作中，原来的字体图标不够用了，需要添加新的字体图标，但是原来的不能删除，继续使用，此时需要这样做
...
在CSS3里使用transition可以实现补间动画（过渡效果），并且当前元素只要有“属性”发生变化时即存在两种状态(用A和B代指），就可以实现平滑的过渡，为了方便演示采用hover切换两种状态，但是并不仅仅局限于hover状态来实现过渡。
...
transformZ的直观表现形式就是大小变化，实质是XY平面相对于视点的远近变化（说远近就一定会说到离什么参照物远或近，在这里参照物就是perspective属性）。比如设置了perspective为200px;那么transformZ的值越接近200，就是离的越近，看上去也就越大，超过200就看不到了，因为相当于跑到后脑勺去了
